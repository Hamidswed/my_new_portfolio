...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch('https://chat-backend-3xpu.onrender.com/api/admin/login', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-2xl w-full max-w-md border dark:border-gray-700">
        <h2 className="text-2xl font-bold text-center text-gray-800 dark:text-white mb-6">🔐 {t('admin.login')}</h2>
